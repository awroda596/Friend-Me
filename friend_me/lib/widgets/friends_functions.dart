...
Future<http.Response> getUsers(
    String? UID, String? QueryType, String? QueryParam) async {
  ...
  if (QueryType == null) {
    ...
    url = Uri(
      ...
      port: port,
      path: path,
      queryParameters: {'$QueryType': '$QueryParam'},
    );
  }
  print(url);
...
List<User> listUsersOffline() {
  ...
  Users.add(User(
      id: 1,
      username: "throwdowntime777",
      email: "bobby@foodnetwork.net",
      first_name: "bobby",
      last_name: "flay"));
  Users.add(User(
      id: 1,
      username: "timmy1992",
      email: "tsmith@mail.com",
      first_name: "Tom",
      last_name: "Smith"));
  Users.add(User(
      id: 1,
      username: "cptrex501st",
      email: "crex501@mail.com",
      first_name: "CT",
      last_name: "7567"));
  Users.add(User(
      id: 1,
      username: "bobthebuilder",
      email: "wecanfixit@mail.com",
      first_name: "Bob",
      last_name: "Builder"));
  Users.add(User(
      id: 1,
      username: "user223",
      email: "user223@mail.com",
      first_name: "User",
      last_name: "Name"));
  Users.add(User(
      id: 1,
...
Future<http.Response> addFriend(String? UID, String? identifier) async {
  ...
  http.Response response = await http.post(
    ...
    body: jsonEncode(<String, String>{
      'target':
          "$identifier", //identifying variable of another user. (username/table id)
    }),
  );
...
List<friendRequest> ListMeetUps(http.Response response) {
  Iterable list = json.decode(response.body);
  List<friendRequest> friendRequests = List<friendRequest>.from(
      list.map((model) => friendRequest.fromJson(model)));
...
class friendRequest {
  ...
  final String
      to_user_id; //redundant, we know it's to us, backend knows from the auth header
